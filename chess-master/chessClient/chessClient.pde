...
void receiveMove() {
  if (myClient.available() > 0) {
    ...
    //println(a);
    //println("gridr1c1:", grid[r1][c1]);
    //println("gridr2c2:", grid[r2][c2]);
...
void mouseReleased() {
  if (firstClick ) {
    ...
    if (!(row2 == row1 && col2 == col1)) {
      ...
      grid[row1][col1] = ' ';
      myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 0 + "," + lastPieceTaken + "," + promoPiece);
      firstClick = true;
      itsMyTurn = false;
      tB = true;
    }
...
void takeBack() {
  if (tB) {
    ...
    myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 1 + "," + lastPieceTaken + "," + promoPiece);
    promo = false;
    tB = false;
  }
}
